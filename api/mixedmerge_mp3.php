 ...
 *
 * IMPORTANT:
 * - inputPrefix is NOT required from client.
 * - Each source path is validated against $ALLOWED_INPUT_PREFIXES.
 */
...
// Validate all sources: safe + allow-listed prefix per item
$srcPaths = [];
$sourcePrefixes = [];
foreach ($sources as $p) {
  ...
  $matchedPrefix = null;
  foreach ($ALLOWED_INPUT_PREFIXES as $prefix) {
...
try {
  ...
  foreach ($srcPaths as $i => $relPath) {
    ...
    curl_setopt_array($ch, [
      ...
      CURLOPT_TIMEOUT => 30,
      CURLOPT_USERAGENT => "mixedmerge_mp3.php/1.0",
    ]);
    $ok   = curl_exec($ch);
  ...
  $resp = [
    "ok" => true,
    "mode" => $mode,
    "sourcePrefixes" => array_values(array_unique($sourcePrefixes)),
    "outputDir" => $outputDir,
